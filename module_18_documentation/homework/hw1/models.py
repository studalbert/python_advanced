...
def load_swagger_json(file_path):
    """Загрузка Swagger документации из JSON файла."""
    with open(file_path, "r") as f:
        return json.load(f)


def swag_from_json(file_path):
    """Обёртка над swag_from для использования JSON файлов."""
    swagger_doc = load_swagger_json(file_path)

    def decorator(func):
        return swag_from(swagger_doc)(func)
